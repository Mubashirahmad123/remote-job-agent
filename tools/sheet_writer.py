...
def append_rows(rows):
    ...
    try:
        ...
        for i, job in enumerate(rows, 1):
            ...
            job_url = job.get("apply_url", "").split("?")[0].strip()
...
def append_rows_simple(rows):
    ...
    try:
        ...
        if new_rows:
            # Add timestamp to first job as comment
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            if new_rows:
                new_rows[0][-1] += f" [Added: {timestamp}]"  # Add to summary
...
def get_all_rows():
    ...
    """
    try:
        sheet = get_or_create_worksheet(get_sheet(), "LIVE Remote Jobs Tracker")
